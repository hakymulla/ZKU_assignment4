...
contract RollupNC is Update_verifier, Withdraw_verifier{
    ...
    // The update function takes as input the snark proof and does some verification
    // it verifies that the current root from the snark proof is the same as the one on the contract
    // it also verifies if the snark proof is valid
    // after verification, it updates the current merkle root 
    // increments the global variable updateNumber
    // update the updates mapping with the new update number (txRoot => update idx)
    // then emits the transaction for logging
    ...
    // the deposit function takes 3 inputs, a public key, amount and tokentype
    // it checks for three type of tokens
    // if token type is 0
    ////// validates if the sender is the coordinator(contract creator)
    ////// also validates if the amount passed and the value is zero (it has no value)
    // if token type is 1
    ////// it validates that the sender has a value greater than 0 and the amount passed in is greater than zero
    // if token type is greater than 1
    ////// it validates if the amount passed is greater than zero
    // IERC20 is called with the tokentype address and check if the user and the stated amount can transfer token
    // A depositArray array is created and updated with the user inputs (public key, amount and tokentype) and hashed with hashMiMC
    // the hash is added to the pendingDeposits (some sort of queue)
    // and the emit function is called
    // the global variable queueNumber is incremented
    // the while loop to check if the queueNumber is even, if this is true
    // the last 2 items in pendingDeposits is hashed and updated accordingly
    // the global depositSubtreeHeight is also updated
    ...
    // The withdram function takes 7 inputs
    // The following validation checks are done
    // 1. checks if the token is not zero, i.e the token has real value
    // 2. checks if the txRoot is valid in updates mapping, i.e the index is greater than zero
    // if check 2 passes, the transaction information is hashed
    // 3. micmMerkle getRootFromProof is called with the appropriate parameters and is checked against the  txRoot
    // 4. eddsa signature verification is done with the withdraw_verifyProof function
    // if all check passes, the toke is then transfered
