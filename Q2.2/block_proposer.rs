...
// create_mempool_req functions has two input last_priority_op_number and block_timestamp and returns the MempoolBlocksRequest and the receiver.
...
// BlockProposer has four data types current_priority_op_number, root_hash_queue, mempool_requests, statekeeper_requests,
...
// A class implementation of BlockProposer has an async method called run that takes as input miniblock_interval.
// This method commits new transaction mini block, also check if there are too many blocks in which the root hash is not calculated and postpones the miniblock. 
// A method called propose_new_block that returns a ProposedBlock.
// It calls the create_mempool_req function and send the mempool request.
// A method called get_pending_block_timestamp that sends the block_timestamp_sender to the statekeeper
// A method commit_new_tx_mini_batch that mutates its state.
// Gets the pending block timestamp and a proposed block.
// Increments current_priority_op_number and commits the proposed_block as a miniblock
